...
class AistudioAccount {
  ...
  constructor({ accountKey, cookieStr, cfg, log }) {
    ...
    this.waiters = [];         // 捕获槽：[{predicate, resolve, timer}]
  }
  ...
  async reloadCookies() {
    ...
    const ctxCookies = this.context.contexts ? undefined : undefined; // noop 占位
  ...
  async _handleRpc(route) {
    ...
    // 喂给等待中的任务（配对后移除；没有 waiter 的响应是页面自己跑的，丢弃）
    const w = this.waiters.shift();
    if (w) {
      clearTimeout(w.timer);
      w.resolve({ status: resp.status(), bytes: buf });
    }
