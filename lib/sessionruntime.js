...
const last = {
  ...
  collectAt: 0,     // 最近一次账号池采集（积分等）的时间
  collectOk: null,  // 那次采集是否走通（号池拿不到清单 / 全部账号都失败 ⟹ false）
  collectCount: 0,  // 那次实际采到并回报成功的账号数
  collectError: '',
};
...
function isAuthExpired(err) {
  // 业务码是**硬事实**，优先认它。message 正则保留作兜底（老调用方只拼字符串）。
  // 别只留正则：措辞是会变的，码不会。
  if (err && String(err.upstreamCode || '') === String(LOGIN_REQUIRED_CODE)) return true;
...
/**
 * 空转期采集 —— 把号池里**每个账号**都验一遍、采一遍（积分 / 额度 / 并发上限）。
 *
 * 为什么需要它（而不是「等跑任务时顺手采」）：
 * 号池在北京够不到 `ads.tiktok.com`，控制台上「这个号还剩多少积分」只能由
 * 节点采。而节点原本只在 `lease` 那一刻才知道自己拿到哪个号 —— 于是**刚加完
 * 号、还没跑过任务**的账号，积分栏会一直空着。用户看到的就是「我加了号，
 * 但积分不显示」，然后开始怀疑自己是不是没接上。
 *
 * 一次采集对每个账号打 4 个**免费只读**接口（验活 + 积分 + 额度 + 并发上限），
 * 合成一次 `reportAccount` 回报。所以：
 *
 * - **不占槽位**：走 `GET /accounts`（只读清单），不是 `lease`。
 *   用 lease 会让「采集」把并发槽占满，任务反而租不到号。
 * - **一个账号失败不影响其它账号**：某个号 cookie 死了，就那一个号回报
 *   `ok:false`，其余照常。整批中断等于让一个坏号挡住全池。
 * - **验活自身出错时不改判结论**：网络不通 ≠ cookie 失效。这种情况只报
 *   `info`（不带 ok/code），号池那边就只更新快照、不动验活结论 ——
 *   把网络抖动写成「凭据失效」会让人去换一份根本没问题的 cookie。
 */
async function collectPool(client, log, { backend = cfg.sessionBackend, fetchImpl = null } = {}) {
  const tiktok = require('./tiktok');
  const key = backend || cfg.sessionBackend;

  let r;
  try {
    r = await client.accounts(key);
  } catch (err) {
    last.collectAt = Date.now();
    last.collectOk = false;
    last.collectError = err.message;
    // 404 单独说一句：那几乎一定是**号池镜像太旧**（还没这条端点），
    // 和「号池挂了」的处置完全不同（升级号池 vs 等它恢复）。
    if (err && err.status === 404) {
      log('号池没有 /api/v1/agent/accounts 这条端点（HTTP 404）—— '
        + '多半是号池镜像还是旧版；升级号池后即可自动采集积分', 'warn');
    } else {
      log(`账号清单拉取失败（${err.message}）`, 'warn');
    }
  ...
  if (!list.length) {
    ...
    last.collectCount = 0;
    last.collectError = '';
    log('号池里没有 active 账号（或都没凭据），跳过采集', 'debug');
  ...
  for (const acc of list) {
    ...
    let session;
    try {
      session = normalizeSession(acc.session);
    } catch (err) {
      log(`账号 #${acc.id} 凭据结构不对（${err.message}）—— 跳过采集`, 'warn');
      continue;
    }
    ...
    let probe = null;
    try {
      probe = await tiktok.probeSession(session, cfg, { fetchImpl });
    } catch (err) {
      log(`账号 #${acc.id} 验活异常（不改判结论）：${err.message}`, 'warn');
    }

    // ---- 采集快照（内部已保证单项失败不致命）----
    const info = await tiktok.collectAccountInfo(session, cfg, { log, fetchImpl });

    // ---- 回报 ----
    const body = { agent_id: cfg.agentId, account_id: acc.id, info };
    if (probe) {
      body.ok = probe.alive;
      body.code = probe.code;
      body.message = String(probe.message || '').slice(0, 300);
      if (probe.alive) alive += 1;
    }
    try {
      await client.reportAccount(body);
      done += 1;
      const credit = (info.credits !== undefined || info.bonus !== undefined)
        ? `${(Number(info.credits) || 0) + (Number(info.bonus) || 0)} 分` : '积分未采到';
      log(`账号 #${acc.id}${acc.label ? `（${acc.label}）` : ''} 采集完成：`
        + `${probe ? (probe.alive ? '会话有效' : `会话失效 code=${probe.code}`) : '验活未得结论'}`
        + ` · ${credit} · 并发上限 ${info.max_count === undefined ? '?' : info.max_count}`,
      probe && probe.alive === false ? 'warn' : 'info');
    } catch (err) {
      log(`账号 #${acc.id} 采集回报失败（不影响其它账号）：${err.message}`, 'warn');
    }
  }

  last.collectAt = Date.now();
  last.collectOk = done > 0;
  last.collectCount = done;
  last.collectError = done ? '' : '全部账号回报失败';
  return { ok: done > 0, count: done, alive, total: list.length };
}

/**
 * 定时采集。
 *
 * 频率的取舍：这是对 `ads.tiktok.com` 发真请求，一个账号一轮 4 个接口。
 * 太密（比如跟着会话刷新的 10 分钟走）会打出一串很有规律的流量；
 * 而这个数据的用途是「看板别显示三天前的数字」，半小时的粒度完全够用。
 * 所以默认 30 分钟，且**下限硬夹在 5 分钟**。
 */
function startCollectTimer(client, log, seconds) {
  if (collectTimer) { clearInterval(collectTimer); collectTimer = null; }
  const raw = Number(seconds === undefined || seconds === null
    ? cfg.accountCollectSeconds : seconds);
  // ≤0 = 显式关掉定时采集（启动那一轮仍然会跑一次，那个是「加了号立刻能看到」的保证）
  if (Number.isFinite(raw) && raw <= 0) return;
  const sec = Math.max(300, raw || 1800);
  collectTimer = setInterval(() => {
    collectPool(client, log, { backend: cfg.sessionBackend })
      .catch((err) => log(`定时采集出错：${err.message}`, 'warn'));
  }, sec * 1000);
  if (collectTimer.unref) collectTimer.unref();
}
...
async function start(client, log) {
  ...
  startCollectTimer(client, log);

  // 首次采集**故意不 await**：它要对每个账号打 4 个只读接口，串起来可能几秒到
  // 十几秒；卡在这里会让节点迟迟不进取活循环，号池那边看到的是「节点启动了但
  // 不领活」。采集是看板用的辅助，绝不能挡住主线。
  collectPool(client, log, { backend: cfg.sessionBackend })
    .then((r) => {
      if (r && r.ok && r.count) {
        log(`首轮账号采集完成：${r.count}/${r.total} 个账号已回报（积分等见控制台）`, 'info');
      }
    })
    .catch((err) => log(`首轮账号采集出错（不影响取活）：${err.message}`, 'warn'));
...
/**
 * 任务终态后的**积分快照补采**（lease 模式专属）。
 *
 * 为什么要有它：控制台「积分合计」读的是节点回报的快照（`info.ts`），
 * 此前只有「启动一轮 + 每 30 分钟定时」两处采集 —— 一次任务扣掉积分后，
 * 看板要滞后最多半小时才追上，对着 TikTok 官方页面对账必然对不上
 * （2026-09-21 实测：快照 4394 vs 页面实时 4354，差的 40 就是刚跑完的 3 单）。
 *
 * 设计约束：
 * - **只报 info，不带 ok/code** —— 号池只更新快照、不动验活结论
 *   （同 collectPool 里「验活异常不改判」的口径，这里压根不验）。
 * - **复用 collectAccountInfo**（4 个只读接口）：积分、每日额度、并发上限
 *   一次全刷新 —— 「克隆剩余」也是每任务都在变的数。
 * - **同账号串行**：并发 worker 租同一个号、任务几乎同时结束时不能打出一串
 *   并发请求 —— 用一条模块级 promise 链排队。
 * - **全程不抛**：失败只落日志，绝不影响任务本身的结果回报。
 */
let infoChain = Promise.resolve();
function refreshAccountInfo(client, session, accountId, log, { fetchImpl = null } = {}) {
  const run = infoChain.then(async () => {
    const tiktok = require('./tiktok');
    let info;
    try {
      info = await tiktok.collectAccountInfo(session, cfg, { log, fetchImpl });
    } catch (err) {
      log(`账号 #${accountId} 任务后快照采集异常（下轮定时采集会补上）：${err.message}`, 'debug');
      return { ok: false, error: err.message };
    }
    if (!info || (info.credits === undefined && info.max_count === undefined
        && !(info.daily && Object.keys(info.daily).length))) {
      return { ok: false, error: 'empty info' };
    }
    try {
      await client.reportAccount({ agent_id: cfg.agentId, account_id: accountId, info });
    } catch (err) {
      log(`账号 #${accountId} 任务后快照回报失败（下轮定时采集会补上）：${err.message}`, 'debug');
      return { ok: false, error: err.message };
    }
    const credit = (info.credits !== undefined || info.bonus !== undefined)
      ? `${(Number(info.credits) || 0) + (Number(info.bonus) || 0)} 分` : '积分未采到';
    log(`账号 #${accountId} 任务后快照已刷新：${credit}`, 'info');
    return { ok: true };
  });
  // 链上失败不能传染给下一次
  infoChain = run.catch(() => {});
