...
/**
 * AI Studio（Google）会话校验 —— 与 TikTok 分档：只要 cookie，且必须带
 * SAPISID（SAPISIDHASH 签名的原料，缺了必 401，存了也是废凭据）。
 * 号池侧 session_cred 已经做过同样的硬校验，这里是节点侧的最后一道闸。
 */
function normalizeAiSession(sess) {
  if (!sess || typeof sess !== 'object') {
    throw new Error('会话凭据解出来不是对象');
  }
  // 登录凭据形态（kind=google_login）：存的不是 cookie，而是节点自登录要
  // 用的材料。执行面用持久 profile（登录态自持），这里只透传字段。
  if (sess.kind === 'google_login') {
    if (!sess.email || !sess.password) {
      throw new Error('登录凭据缺少邮箱/密码');
    }
    return {
      kind: 'google_login',
      email: String(sess.email),
      password: String(sess.password),
      app_password: String(sess.app_password || ''),
      totp_secret: String(sess.totp_secret || ''),
      backup_codes: Array.isArray(sess.backup_codes) ? sess.backup_codes.map(String) : [],
    };
  }
  if (!sess.cookie) throw new Error('Google 会话凭据缺少 cookie');
  const hasSapisid = /(?:^|;\s*)(?:SAPISID|__Secure-[13]PAPISID)=/.test(String(sess.cookie));
  if (!hasSapisid) {
    throw new Error('Google cookie 缺少 SAPISID（生图签名原料，缺了必 401）');
  }
  return { kind: 'cookie', cookie: String(sess.cookie) };
}
