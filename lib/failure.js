...
function build(kind, { code = '', retryable = true, note = '' } = {}) {
  return { kind, code, retryable, note };
}
...
function classify(err) {
  ...
  if (err.cancelled) return build(KIND.CANCELLED, { retryable: false });
  ...
  if (code === SESSION_CODE || SESSION_HINTS.some((h) => text.includes(h))) {
    return build(KIND.SESSION_EXPIRED, { code, retryable: false });
  }

  // ② 内容策略 —— 码前缀或措辞命中任一即算
  if (code.startsWith(CONTENT_CODE_PREFIX) || CONTENT_HINTS.some((h) => text.includes(h))) {
    return build(KIND.CONTENT_MODERATION, { code, retryable: false });
  }

  // ③ 本地自己判的超时（上游还在跑，重发有意义）
  if (err.localTimeout) return build(KIND.TIMEOUT, { code, retryable: true });

  // ④ 额度/并发
  if (QUOTA_HINTS.some((h) => text.includes(h))) {
    return build(KIND.QUOTA, { code, retryable: true });
  }

  // ⑤ 参数类
  if (PARAM_HINTS.some((h) => text.includes(h))) {
    return build(KIND.PARAM, { code, retryable: false });
  }

  // ⑥ 明确的传输层/上游抖动
  if (err.transient || /HTTP 5\d\d|ECONNRESET|ETIMEDOUT|socket hang up/.test(String(err.message || ''))) {
    return build(KIND.UPSTREAM_ERROR, { code, retryable: true });
  }

  return build(KIND.UNKNOWN, { code, retryable: true });
}
...
module.exports = {
  KIND, classify, describe, outcomeOf, tagUpstream, upstreamFailure, LABEL,
};
