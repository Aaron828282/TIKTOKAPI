...
function createClient(cfg) {
  ...
  return {
    ...
    /**
     * 某后端的账号全清单（含凭据，**不占槽位**）。
     * profile 残留对账用：清单里没有的账号，其磁盘 profile 可清。
     */
    listAccounts: (backend) => call(
      `/api/v1/agent/accounts?backend=${encodeURIComponent(backend)}`, { timeout: 20000 }),

    /**
     * 本 agent 名下「执行中但任务心跳停滞」的孤儿单 id 清单。
     *
     * 节点被托管平台杀掉重启（OOM）后，内存里的任务全成孤儿：号池状态停在
     * 「生成中」直到 30 分钟超时。启动时调这个拿清单、逐条回报失败（result
     * 幂等），用户几秒内就能重发。stale=90s：心跳最密 5s 一次，90s 无心跳
     * 只可能是进程没了或网络断了，误伤面为零。
     */
    orphans: (agentId, stale = 90) => call(
      `/api/v1/agent/orphans?agent_id=${encodeURIComponent(agentId)}&stale=${stale}`,
      { timeout: 20000 }),
