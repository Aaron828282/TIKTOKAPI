...
function validate() {
  ...
  if (!hasSession()) {
    ...
    // 拿到手了就顺手算一下「还剩多久」。广告线 TTL 只有 3 天，
    // 「有没有 cookie」远不如「还能用几小时」有用 —— 前者永远为真。
    const d = describe(loadSession());
    if (d.level === 'dead') {
      warn.push(`TikTok 会话：${d.note} —— 每个任务都会在上游拿 10001106，`
        + '必须重新取一份（见 README「拿会话凭据」）');
...
module.exports = {
  cfg, loadSession, hasSession, validate, sessionStatus,
