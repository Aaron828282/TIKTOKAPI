...
const cfg = {
  ...
  // ---- AI Studio 生图（backend=aistudio_image）----
  // 总开关：**默认关闭**。执行面 = Playwright + 每账号常驻 Chromium 持久
  // profile（令牌绑定请求内容，必须由页面现签 —— 见 docs/aistudio-pool-plan.md）。
  // Playwright/Chromium 在托管容器里未必装得上，没实测之前不让生图任务进来：
  // 开关关闭时 RH_BACKENDS 不声明 aistudio_image，claim 根本领不到这类任务，
  // TikTok 视频链路零影响。
  aistudioEnabled: envBool('RH_AISTUDIO_ENABLED', false),
  /** 单账号并行 tab 数（= 单账号并发）。用户定 10；内存吃紧可降（每 tab ~300-500MB）。 */
  aistudioTabs: Math.max(1, Math.min(10, envInt('RH_AISTUDIO_TABS', 10))),
  /** Chromium 持久 profile 根目录（每账号一个子目录，cookie 只落这里，密文之外的地方）。 */
  aistudioProfileDir: env('RH_AISTUDIO_PROFILE_DIR', 'data/aistudio-profiles'),
  /** 单次生图等待上限（4K 实测 ~120s，留一倍余量）。 */
  aistudioTimeoutSeconds: Math.max(60, envInt('RH_AISTUDIO_TIMEOUT_SECONDS', 300)),
  /**
   * 单任务最多交付几张图（上游一次出 1~4 张，控制台无数量开关）。
   * 0 = 全保留（默认）；N = 只取前 N 张交付、其余在节点侧丢弃。
   * 订阅额度内上游照常全量生成，丢弃不产生额外费用。
   */
  aistudioMaxImages: Math.max(0, envInt('RH_AISTUDIO_MAX_IMAGES', 0)),
  /** 在途改写的分辨率档（generationConfig 末位 [null,"1K"|"4K"]，令牌不绑配置）。 */
  aistudioResolution: env('RH_AISTUDIO_RESOLUTION', '4K'),
  ...
  /**
   * 参考视频上传（2026-09-23 打码链路）：VOD Inner 上传通道。
   * ApplyUploadInner/CommitUploadInner（Version 2020-11-19，签名服务名 vod）。
   *
   * 🔴 2026-09-23 真实抓包实锤（ApplyUploadInner 完整请求/响应）：
   * - 网关就是**同域 upload-proxy**（此前探测 404 是因为缺 SpaceName/FileType
   *   等路由参数，并非独立 host）
   * - SpaceName=ad_site；签名 region=i18n、服务名=vod
   * - StoreUri/Auth/SessionKey/Vid 全在 Result.InnerUploadAddress.UploadNodes[0]
   */
  videoHost: env('RH_VIDEO_HOST', 'https://ads.tiktok.com/creative/creativestudio/upload-proxy'),
  videoSpace: env('RH_VIDEO_SPACE', 'ad_site'),
...
// AI Studio 生图通道只在显式开启（RH_AISTUDIO_ENABLED=1）后才会被 claim：
// Playwright/Chromium 没装好之前领了任务也执行不了 —— 不领才是对任务最负责
// 的处理，也保证生图灰度期间 TikTok 视频链路零影响。
if (!cfg.aistudioEnabled) {
  cfg.backends = cfg.backends.filter((b) => b !== 'aistudio_image');
}
