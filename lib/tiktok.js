...
async function poll(session, cfg, taskId, opts = {}) {
  ...
  while (Date.now() - started < timeoutMs) {
    if (onTick) {
      ...
      if (verdict && verdict.stop) throw new Error('任务已被调用方取消');
    }
    ...
    if (done || videoOk) {
      ...
      return {
        ...
        variants: vinfos.map((v) => ({
          url: v.MainUrl,
          width: (v.VideoMeta || {}).Width,
          height: (v.VideoMeta || {}).Height,
          format: (v.VideoMeta || {}).Format,
          codec: (v.VideoMeta || {}).Codec,
          size: (v.VideoMeta || {}).Size,
          expire: v.UrlExpire,
        })),
