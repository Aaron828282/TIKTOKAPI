...
async function ensureLogin({ context, creds, log, shotsDir }) {
  ...
  while (Date.now() < deadline) {
    ...
    // 2SV 选择页：Authenticator → 备用码；第一屏没有就展开 Try another way
    const bodyText = await page.innerText('body').catch(() => '');
    if (/2-Step Verification|Choose how you want to sign in/i.test(bodyText)) {
      selectionTries += 1;
      if (selectionTries > 5) {
...
async function _pickChallengeOption(page, creds, out) {
  ...
  for (const text of want) {
    const opt = await page.$(`div[role="link"]:has-text("${text}"),`
      + ` li[role="link"]:has-text("${text}"), a:has-text("${text}")`);
    if (opt) {
...
module.exports = { ensureLogin, fetchVerificationCode, totpCode, applySpoof, maskEmail };
